...
deploy_dev_job:
  ...
  script:
    ...
    - gcloud compute ssh docker-host --force-key-file-overwrite --command="docker run --rm -d --name reddit -p 9292:9292 kovtalex/reddit"
  environment:
    name: dev
...
В результате будет развернуто dev окружение с нашим приложением в контейнера на виртуальной машине docker-host в GCP
